...
#define setupobjectpropert(o,t, p) asEngine->RegisterObjectProperty( str(#o).c_str() ,  str(str(#t) + " " +  str(#p)).c_str(), asOFFSET(o, p)); assert(r >= 0);
    ...
    /*
     Registering rigidbody object below throws runtime error:

     System function (1,12) : ERR  : Identifier 'string' is not a data type in global namespace
 (0,0) : ERR  : Failed in call to function 'RegisterGlobalFunction' with 'void print(string)' (Code: -10)
-10 SHUTDOWN ENGINE
        ANGEL SCRIPT ENGINE HAS RUN AN ERROR IN THE INTERAL CODE
        IF ERROR CONTINUES PLEASE REDOWNLOAD ENGINE
        Error in file: shared/wings.cpp on line: 360
Property (1,1) : ERR  : Identifier 'string' is not a data type in global namespace
 (0,0) : ERR  : Failed in call to function 'RegisterObjectProperty' with 'node' and 'string name' (Code: -10)
Assertion failed: (r >= 0), function config, file shared/wings.cpp, line 421.
     */
    //r = asEngine->RegisterObjectType("rigidbody", sizeof(rigidbody), asOBJ_VALUE | asOBJ_APP_CLASS_CDAK); assert(r >= 0);
