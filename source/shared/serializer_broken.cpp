...
{
	const char *typeName;
	if (typeId <= asTYPEID_DOUBLE) typeName = asScript->getprimitivename(typeId).c_str();
	else
	{
...
struct DataHold
{
	...
	{
		str buff;
		if (nameID <= asTYPEID_STRING) 
		{
			buff.resize(data.size());
			memcpy(&buff[0], &data[0], data.size());
			conoutf("%s%s %s %s", namespaces[VECTOROVERLOADCHECK(namespaces, namespaceID)] == "" ? "" : (namespaces[VECTOROVERLOADCHECK(namespaces, namespaceID)] + "::").c_str(), typeID > asTYPEID_DOUBLE ? types[VECTOROVERLOADCHECK(types, typeID - (asTYPEID_DOUBLE + 1))].c_str() : asScript->getprimitivename(typeID).c_str(), names[VECTOROVERLOADCHECK(names, nameID)].c_str(), buff.c_str() );
		}else
			conoutf("%s%s %s %d", namespaces[VECTOROVERLOADCHECK(namespaces, namespaceID)] == "" ? "" : (namespaces[VECTOROVERLOADCHECK(namespaces, namespaceID)] + "::").c_str(), typeID > asTYPEID_DOUBLE ? types[VECTOROVERLOADCHECK(types, typeID - (asTYPEID_DOUBLE + 1))].c_str() : asScript->getprimitivename(typeID).c_str(), names[VECTOROVERLOADCHECK(names, nameID)].c_str(), children);
		
	}
	void write(stream *f)
	{
		f->putlil<ushort>(namespaceID);
		f->putlil<ushort>(typeID);
		f->putlil<ushort>(nameID);
		f->putlil<ushort>(children);
		if (typeID == asTYPEID_STRING) { f->write(&data[0], children); return; }
	...
	{
		uint nsp = f->getlil<ushort>();
		uint tp = f->getlil<ushort>();
		uint nm = f->getlil<ushort>();
		DataHold *temp = new DataHold(nsp, tp, nm);
		temp->children = f->getlil<ushort>();
		if (temp->typeID < asTYPEID_STRING && temp->typeID != asTYPEID_VOID)
		{
			...
			temp->data.resize(temp->children);  f->read(&temp->data[0], temp->children);
			//str buff;
			//buff.resize(temp->data.size());
			//memcpy(&buff[0], &temp->data[0], temp->data.size());
			//conoutf("hello there %s", buff);
		}
		return temp;
	}
...
int gettypeid(str name) { GETID(types, asTYPEID_DOUBLE + 1); }
...
void sort() //take the array and put it in a tree structure
{
...
void CSerializer::load(stream *f)
{
	...
	//conoutf("%d", saveddata[0]->children);
	m_root.Uninit();
	m_root.Init();
...
void CSerializedValue::load()
{
	...
	m_isInit = true;
	if (sd.typeID == asTYPEID_VOID) return;
		...
		if (ot)
		{
			...
			if (m_typeId & asTYPEID_SCRIPTOBJECT)
			{
				...
				loopi(m_children.size())
				{
					...
					{
						if (str(m_children[i]->m_name) != str(a->GetPropertyName(i)) || m_children[i]->GetTypeId() != a->GetPropertyTypeId(i))continue;
			...
			else
			{
				...
				CUserType *type = m_serializer->m_userTypes[typeidtoname(m_typeId)];
				if (type)
					type->Create(this);
			}
			//if (m_serializer->m_userTypes != NULL) if(m_serializer->m_userTypes[ot->GetName()]) conoutf("create object");
		}
	}
	else //ok it must be a preimitive
	{
		if (sd.typeID == asTYPEID_STRING) {
			...
			str buff;
			//SetUserData(new std::string("hello"));
			//buff.resize(sd.data.size());
			//memcpy(&buff[0], &sd.data[0], sd.data.size());
			//conoutf("your name is %s", buff.c_str());
			//m_restorePtr = new str;
			//m_restorePtr = (void *)&buff;
			//CSerialstring::Create(this);
		}
		else {
			m_typeId = sd.typeID;
...
void CSerializedValue::save()
{
	...
	DataHold *data = new DataHold(getnamespaceid(m_nameSpace), m_typeId == CSerialstring::getID() ? asTYPEID_STRING : m_typeId > asTYPEID_STRING ? gettypeid(m_typeName) : m_typeId, getnameid(m_name));
	//conoutf("%d %d %s", m_typeId, CSerialstring::getID(), m_name.c_str());
	saveddata.put(data);
	//data->print();
	//conoutf("typeID: %d nameID: %d namespaceID: %d", typeID, nameID, namespaceID);
	...
	else if (m_typeId == CSerialstring::getID())
	{
		//use the usertype to store for now or find a way to store as an array of data
		//SetUserData(new std::string("hello"));
		std::string *buffer = (std::string*)m_restorePtr;
		if(buffer) conoutf("%s saving", buffer->c_str() );
		else conoutf("fail");
		//data->children = ststr.length();
		//conoutf("%d %d hello", data->children, ststr.length());
		//data->data.resize(data->children);
		//memcpy(&data->data[0], &ststr, ststr.length());
		//conoutf("%d %s hello", data->data[0], ststr.c_str());
		//return;
	}
	else {
		data->children = m_children.size();
		loopi(m_children.size()) m_children[i]->save();
	}
	//loopi(m_children.size())
	//{
	//	m_children[i]->save();
	...
	//if (m_typeId & asTYPEID_OBJHANDLE || m_typeId & asTYPEID_SCRIPTOBJECT) return;
	//int size = m_serializer->m_engine->GetSizeOfPrimitiveType(m_typeId);
	//str databuffer;
	//loopi(size) databuffer += m_mem[i];
	//::print(databuffer);
...
//////////////////////////////////////////////////////
#define CSERIALDefine(n,c,s,r,d)\
void CSerial##n::Create(CSerializedValue *val){c}\
void CSerial##n::Store(CSerializedValue *val, void *ptr){s}\
void CSerial##n::Restore(CSerializedValue *val, void *ptr){r}\
void CSerial##n::CleanupUserData(CSerializedValue *val){d}\
int CSerial##n::serialID = -1;
...
{ //create
	...
	//get the transform
 	if (val->m_children.size() < 2)
		...
		//sconoutf("%s", n->o.c_str());
		if (val->m_children.size() > 2)
			//if (!val->m_children[2]->GetUserData()) n->name = "unnmaed";
			//else 
			{
				//td::string *buffer = (new std::string("hello"));
				//n->name = *buffer;
			}
	}
	//add the controlers
	if (val->m_children.size() > 3)
	{
		...
		{
			asIScriptObject *c = (asIScriptObject *) val->m_children[k + 2]->m_restorePtr;
			if (c) for (int i = 0; i < c->GetPropertyCount(); i++)
			{
				...
				else { conoutf("error assigning Self"); continue; } //class had no declared self @node we need to remove this object from the node;
			}
		}
	}
...
val->m_children.push_back(new CSerializedValue(val, "name", "", new str(g->name), CSerialstring::getID()));
...
loopi(g->ctrl.length()) val->m_children[3]->Restore(g->ctrl[i], g->ctrl[i]->GetTypeId());
