...
//new loop type offset used for looping from point to max rather than from 0 to max
#define loopioff(v, m) for(int i = int(v); v < int(m); ++i)
#define loopkoff(v, m) for(int k = int(v); v < int(m); ++k)
#define loopjoff(v, m) for(int j = int(v); v < int(m); ++j)
...
#define defformatstring(d,...) cubestr d; formatstring(d, __VA_ARGS__)
#define defvformatstring(d,last,fmt) cubestr d; { va_list ap; va_start(ap, last); vformatstring(d, fmt, ap); va_end(ap); }
...
inline char *newstring(const cubestrslice &s) { return newstring(s.str, s.len); }
inline const char *stringptr(const char *s) { return s; }
inline const char *stringptr(const cubestrslice &s) { return s.str; }
inline int cubestrlen(const char *s) { return int(strlen(s)); }
inline int cubestrlen(const cubestrslice &s) { return s.len; }

inline char *copystring(char *d, const cubestrslice &s, size_t len)
{
...
template<size_t N> inline char *copystring(char (&d)[N], const cubestrslice &s) { return copystring(d, s, N); }
