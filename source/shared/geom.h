...
//can use in c++ 11;
//#define ALLOPERATORS(c) { TEMPOPERATOR(vec, (*this), += , add);\
...
struct vec
{
	...
	vec() {}
	explicit vec(const btVector3 &v) : vec(v.getX(), v.getZ(), v.getY()) {}
	...
	vec &clamp(vec l, vec h) { x = ::clamp(x, l.x, h.x); y = ::clamp(y, l.y, h.y); z = ::clamp(z, l.z, h.z); return *this;}
	...
	static vec hexcolor(int color) {return vec(((color >> 16) & 0xFF)*(1.0f / 255.0f), ((color >> 8) & 0xFF)*(1.0f / 255.0f), (color & 0xFF)*(1.0f / 255.0f));}
...
struct quat : vec4
{
	...
	quat(const btQuaternion &v) : vec4(v.getX(), v.getZ(), v.getY(), v.getW()) {}
...
struct matrix3
{
    ...
    //angelo sauer phys enhance
    void setpitch(float ck, float sk)
    {
        a = vec(ck, sk, 0);
        b = vec(-sk, ck, 0);
        c = vec(0, 0, 1);
    ...
    {
        setpitch(cosf(angle), sinf(angle));
    }

    void setroll(float ck, float sk)
    {
        a = vec(ck, sk, 0);
        b = vec(-sk, ck, 0);
        c = vec(0, 0, 1);
