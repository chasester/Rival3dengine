...
void collectlights()
{
	...
	if (PartyTime) {
		...
		else shoulddeletenext = false;
	}

	vector<light *> li = curworld->getlights();
	loopv(li)
	{
		const light *l = li[i];
		if (l->radius < 1) continue;
		if (smviscull)
		{
			if (isfoggedsphere(l->radius, l->o)) continue;
			if (pvsoccludedsphere(l->o, l->radius)) continue;
		}
		lightinfo &ml = lights.add(lightinfo(l->o, l->color, l->radius));
		if (ml.validscissor()) lightorder.add(lights.length() - 1);
	}
    int numdynlights = 0;
    if(!drawtex)
    {
...
{
    if(rendernogi(worldeditor::editroot, octaoffset, worldsize>>1, ivec(bbmin).max(nogimin), ivec(bbmax).min(nogimax), minsize))
