...
Shader *newshader(int type, const char *name, const char *vs, const char *ps, Shader *variant = NULL, int row = 0)
{
    ...
    if(!s.compile())
    {
        ...
        logoutf("Failed compiling shader %s", name);
        return NULL;
    }
...
void setupshaders()
{
    ...
    if(!hudshader)fatal("failed to setup nullshader");
    if(!hudnotextureshader)fatal("failed to setup nullshader");
    if(!hudtextshader)fatal("failed to setup nullshader");
