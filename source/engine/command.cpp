...
template<class T> static inline ident *newident(const T &name, int flags)
{
    ...
    if(!id)
    {
        if(checknumber(name))
        {
            debugcode("number %.*s is not a valid identifier name", cubestrlen(name), stringptr(name));
            return dummyident;
        }
...
static inline void cutstring(const char *&p, cubestrslice &s)
{
    p++;
...
ICOMMAND(codestr, "i", (int *i), { char *s = newstring(1); s[0] = char(*i); s[1] = '\0'; cubestrret(s); });
ICOMMAND(struni, "si", (char *s, int *i), intret(*i > 0 ? (memchr(s, 0, *i) ? 0 : cube2uni(s[*i])) : cube2uni(s[0])));
ICOMMAND(unistr, "i", (int *i), { char *s = newstring(1); s[0] = uni2cube(*i); s[1] = '\0'; cubestrret(s); });
