...
namespace mpr
{
    ...
    struct EntOBB : Ent
    {
	  ...
	  //angelo sauer ents
            vec n = orient.transform(wn).div(vec(ent->xradius + zmargin, ent->yradius + zmargin, ent->zradius + zmargin)),//(ent->aboveeye + ent->eyeheight + zmargin))),///2)),
          //angelo sauer ents
                dir = orient.transform(wdir),
                //an(fabs(n.x), fabs(n.y), fabs(n.z)),// dir.z ? fabs(n.z) : 0),
                an(dir.x ? fabs(n.x) : 0, dir.y ? fabs(n.y) : 0,dir.z ? fabs(n.z) : 0),
                fn(0, 0, 0);
            if(an.x > an.y)
            {
            ...
            return orient.transposedtransform(fn);//fn
        }

        vec localsupportpoint(const vec &ln) const
        {
            return vec(ln.x > 0 ? ent->xradius : -ent->xradius - zmargin,
                       ln.y > 0 ? ent->yradius : -ent->yradius - zmargin,
                       //ln.z > 0 ? ent->aboveeye : -ent->eyeheight);
		       //angelo sauer ents
		       ln.z > 0 ? ent->zradius : -ent->zradius - zmargin);
	               //ln.z > 0 ? ent->aboveeye : -ent->eyeheight - zmargin);//without this box/barrel wont ride on plats/elevators
	               //angelo sauer ents
        }
        ...
        float bottom() const { return supportcoordneg(orient.c) + ent->o.z; }
        //angelo sauer ents
        //float top() const { return ent->o.z + ent->aboveeye; }
        float top() const { return supportcoord(orient.c) + ent->o.z; }
    };
    ...
    struct Model
    {
        ...
        Model(const vec &ent, const vec &center, const vec &radius, int yaw, int pitch, int roll) : o(ent), radius(radius)
        {
            ...
            if(pitch) orient.rotate_around_x(sincosmod360(-pitch));//was -
