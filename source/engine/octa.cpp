...
cube *cuberoot = newcubes(F_SOLID);
int allocnodes = 0;
...
cube &lookupcube(const ivec &to, int tsize, ivec &ro, int &rsize)
{
    ...
    cube *c = &cuberoot[octastep(tx, ty, tz, scale)];
    if(!(csize>>scale)) do
    {
        if(!c->children)
        {
...
int lookupmaterial(const vec &v)
{
    ...
    cube *c = &cuberoot[octastep(o.x, o.y, o.z, scale)];
    while(c->children)
    {
        scale--;
...
void remip()
{
    ...
    loopi(8)
    {
        ...
        remip(cuberoot[i], o, worldsize>>2);
    }
    calcmerges();
}
...
void genmerges(cube *c = cuberoot, const ivec &o = ivec(0, 0, 0), int size = worldsize>>1)
{
    ...
    loopi(8)
    {
        ...
        if((size == 1<<maxmerge || c == cuberoot) && cpolys.numelems)
        {
