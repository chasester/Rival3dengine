...
{
    clipplanes &p = clipcache[int(&c - cuberoot)&(MAXCLIPPLANES-1)];
...
static inline bool octacollide(physent *d, const vec &dir, float cutoff, const ivec &bo, const ivec &bs)
{
    ...
    if(diff&~((1<<scale)-1) || uint(bo.x|bo.y|bo.z|bs.x|bs.y|bs.z) >= uint(worldsize))
       return octacollide(d, dir, cutoff, bo, bs, cuberoot, ivec(0, 0, 0), worldsize>>1);
    const cube *c = &cuberoot[octastep(bo.x, bo.y, bo.z, scale)];
